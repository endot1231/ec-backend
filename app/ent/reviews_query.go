...
func (rq *ReviewsQuery) QueryUser() *UsersQuery {
...
func (rq *ReviewsQuery) WithUser(opts ...func(*UsersQuery)) *ReviewsQuery {
	query := (&UsersClient{config: rq.config}).Query()
...
func (rq *ReviewsQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Reviews, error) {
	...
	if query := rq.withUser; query != nil {
		if err := rq.loadUser(ctx, query, nodes,
			func(n *Reviews) { n.Edges.User = []*Users{} },
			func(n *Reviews, e *Users) { n.Edges.User = append(n.Edges.User, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReviewsQuery) loadUser(ctx context.Context, query *UsersQuery, nodes []*Reviews, init func(*Reviews), assign func(*Reviews, *Users)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Reviews)
	nids := make(map[int]map[*Reviews]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(reviews.UserTable)
		s.Join(joinT).On(s.C(users.FieldID), joinT.C(reviews.UserPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(reviews.UserPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(reviews.UserPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Reviews]struct{}{byID[outValue]: {}}
