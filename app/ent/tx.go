...
func (tx *Tx) init() {
	tx.Admins = NewAdminsClient(tx.config)
	tx.Company = NewCompanyClient(tx.config)
	tx.ProductBrands = NewProductBrandsClient(tx.config)
	tx.ProductCategories = NewProductCategoriesClient(tx.config)
	tx.ProductColors = NewProductColorsClient(tx.config)
	tx.ProductSizes = NewProductSizesClient(tx.config)
	tx.ProductStock = NewProductStockClient(tx.config)
...
// applies a query, for example: Admins.QueryXXX(), the query will be executed
