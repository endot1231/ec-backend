...
func (uq *UsersQuery) QueryReviews() *ReviewsQuery {
...
func (uq *UsersQuery) WithReviews(opts ...func(*ReviewsQuery)) *UsersQuery {
	query := (&ReviewsClient{config: uq.config}).Query()
...
func (uq *UsersQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Users, error) {
	...
	if query := uq.withReviews; query != nil {
		if err := uq.loadReviews(ctx, query, nodes,
			func(n *Users) { n.Edges.Reviews = []*Reviews{} },
			func(n *Users, e *Reviews) { n.Edges.Reviews = append(n.Edges.Reviews, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UsersQuery) loadReviews(ctx context.Context, query *ReviewsQuery, nodes []*Users, init func(*Users), assign func(*Users, *Reviews)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(users.ReviewsTable)
		s.Join(joinT).On(s.C(reviews.FieldID), joinT.C(users.ReviewsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(users.ReviewsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(users.ReviewsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Reviews](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
